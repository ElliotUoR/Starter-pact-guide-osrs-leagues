...
export default function ChecklistItem({ step, checked, onToggle, compact = false }: Props) {
  ...
  const content = (
    <>
      ...
      {isCheckable ? (
        <CheckboxIndicator
          checked={checked}
          checkBorder={styles.checkBorder}
          checkFill={styles.checkFill}
          compact={compact}
        />
      ) : (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className={`text-sm leading-snug ${styles.text} ${checked ? 'line-through decoration-1' : ''} ${compact ? 'text-xs' : ''}`}>
            {renderTextWithLinks(step.text, step.links)}
          </span>

          {/* Relic images */}
          {step.relics && step.relics.length > 0 && (
            <span className="flex items-center gap-1.5 ml-4">
              {step.relics.map((filename) => (
                <img
                  key={filename}
  ...
  if (isCheckable) {
    return (
      <button
        ...
        className={`
          w-full text-left flex items-start gap-3 rounded-md border px-3 transition-all duration-200 cursor-pointer select-none
          ${styles.border}
          ${checked ? 'opacity-40' : ''}
  ...
  return (
    <div className={`flex items-start gap-3 rounded-md border px-3 ${compact ? 'py-1.5' : 'py-2.5'} ${styles.border}`}>
      {content}
    </div>
  );
}
